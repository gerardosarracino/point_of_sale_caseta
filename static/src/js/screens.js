...
var ProductListWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.click_product_handler = function(){
            console.log('THE PRODUCT LIST  PRODUCTO--')
            contador++;
            if (contador > 1) {
                // alert('No puedes agregar mas de 1 item!!');
            }   else if (contador == 1) {
                var product = self.pos.db.get_product_by_id(this.dataset.productId);
                options.click_product_action(product);

                /* AQUI DEBERIA DE REALIZARSE LA ACCION DE LLEVAR DIRECTAMENTE AL TICKET  */
    ...
    click_product: function(product) {
       ...
       if(product.to_weight && this.pos.config.iface_electronic_scale){
           this.gui.show_screen('scale',{product: product});
            ...
            if (product.display_name == "RESIDENTE" || product.display_name == "RESIDENTE + 1 EJE" ||
            product.display_name == "RESIDENTE + 2 EJES"){
                var uid = "";
                var ticket_id = "";
                var date = new Date();
                ...
                // AGREGA EL METODO DE PAGO DE EFECTIVO AUTOMATICAMENTE
...
var PaymentScreenWidget = ScreenWidget.extend({
    ...
    renderElement: function() {
        ...
        this.$('.back').click(function(){
            // contador = 0 // REGRESA EL CONTADOR DE UN SOLO ITEM
            self.click_back();
        });
