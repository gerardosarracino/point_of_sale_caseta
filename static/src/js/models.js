...
exports.PosModel = Backbone.Model.extend({
...

    {
        ...
        loaded: function(self,configs, carril){
            ...
            //console.log(' CARRIL BUSQUEDA xx', self.config.carril);
            self.carril = self.config.carril;
            //self.db.save('carril', self.config.carril)
...
exports.Order = Backbone.Model.extend({
    initialize: function(attributes,options){
        ...
        //this.carril = this.pos.pos_session.config_id.carril;
        ...
        if (options.json) {
            ...
            this.name = _.str.sprintf(this.uid); // MODIFICACION DE PREFIJO AQUI this.name = _.str.sprintf(_t("Order %s"), this.uid);
    ...
    init_from_JSON: function(json) {
        ...
        this.uid = json.uid;
        this.name = _.str.sprintf(this.uid); // _t("A %s"),
    ...
    generate_unique_id: function() { // ID UNICA DE LA ORDEN DE PAGO
        ...
        //alert(zero_pad(this.pos.pos_session.config_id.name));
        //alert(this.pos.config.carril);
                ...
                minute: date.getMinutes() ,
                isostring: date.toISOString(),
                localestring: this.formatted_validation_date, time.datetime_to_str(new Date()) */

        var date = new Date();
        console.log(Math.random().toFixed(2)*100, 'mm ', date.getSeconds());

        return "0" + this.pos.config.carril + date.getFullYear() +("0" + (date.getMonth() + 1)).slice(-2) + date.getDate() +
        date.getHours() + date.getMinutes() + date.getSeconds() + parseInt(Math.random().toFixed(2)*100);
        // return zero_pad(this.pos.pos_session.login_number,3) + zero_pad(Math.random().toString(36).substr(2, 9));
       // zero_pad(this.pos.pos_session.id,5) +'-'+ zero_pad(this.pos.pos_session.login_number,3) +'-'+ zero_pad(this.sequence_number,4);
    },
    get_name: function() {
        return this.name;
    },
